...
| # | 課題 | 実装場所 | コスト |
|---|---|---|---|
| 1 | 詳細ページのアクションバー（わかる！/保存/シェア）をモバイルでスティッキー化 | `global.css` + `.story-actions` | 小 |
| 2 | 投稿完了画面にPROJECT 100誘導を追加（「あなたが{N}人目！」） | `submit.astro` + `stories.json`からrealCount参照 | 小 |
| 3 | Xシェアテキストの先頭にcatchphraseを追加してフック化 | `[id].astro` の `shareText` 変数 | 小 |

### 🟠 中優先度（体験の質・CVに影響）

| # | 課題 | 実装場所 | コスト |
|---|---|---|---|
| 4 | NARUTOタグはオレンジ、BORUTOタグはブルー系に色分け | `global.css` `.tag` | 小 |
| 5 | トップヒーローに「🎲 ランダムに1件読む」軽量CTAを追加 | `index.astro` + `global.css` | 小 |
| 6 | 「わかる！」ボタンにpulseアニメーション（初見に機能を気づかせる） | `global.css` | 小 |
| 7 | フィルター件数変化時にオレンジフラッシュのフィードバック | `stories/index.astro` | 小 |

### 🟡 低優先度（ブランド・長期品質）

| # | 課題 | 実装場所 | コスト |
|---|---|---|---|
| 8 | hero-metaの「{N}ヶ国から」を `/data` ページへのリンクにする | `index.astro` | 小 |
| 9 | 実投稿25件超えでシードコンテンツを一覧から非表示（前倒し検討） | `StoryCard.astro` + `stories/index.astro` | 中 |
| 10 | 投稿フォームをGoogleフォームからTally.so（`dWl9Lz`）に統一 | `submit.astro` | 要確認 |

### 📌 その他継続課題

- カレンダーページ データ整備中（`src/data/events.ts` に追記後、ナビリンクを復活させて公開）
- ヒーロースニペット（多言語トップ `/[lang]/`）未実装。現在は日本語トップのみ
- `getCharaName()` 未登録のキャラは日本語にフォールバック。新キャラ追加時は `src/i18n/ui.ts` の `CHARA_TRANSLATIONS` に追記が必要
- ページネーション未実装（件数が増えたタイミングで対応）
- Instagramストーリーズ画像のアラビア語対応：Canvas 2D API がデフォルトLTR描画のためアラビア語（RTL）は正常出力不可
