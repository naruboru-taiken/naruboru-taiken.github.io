...
// ── フォント読み込み（ビルド時にnode_modulesから取得）──
  ...
  return h('div', {
    ...
    h('div', {
      ...
      h('div', {
        ...
        h('span', { style: { color: '#fff', fontSize: '32px', fontWeight: 700 } }, 'ナルボル体験談')
      ),
      // 区切り線
      h('div', {
        style: {
      ...
      h('div', {
        ...
        h('div', { style: { display: 'flex', alignItems: 'center', gap: '12px' } },
          h('span', { style: { color: 'rgba(255,255,255,0.65)', fontSize: '24px' } },
            `— ${story.displayName} さん`
          ),
          pill(story.firstSeries, '#e8621a'),
...
export const GET: APIRoute = async ({ props }) => {
  ...
  const svg = await satori(buildCard(story as any), {
    width: 1200,
    height: 630,
    fonts,
  });
